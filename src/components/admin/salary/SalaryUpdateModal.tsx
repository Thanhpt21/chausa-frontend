...
import { useUpdateSalary } from '@/hooks/salary/useUpdateSalary';
...
export const SalaryUpdateModal = ({
  open,
  onClose,
  salary,
  refetch,
  ...
  const { mutateAsync, isPending } = useUpdateSalary();
  ...
  useEffect(() => {
    if (salary && open) {
      form.setFieldsValue({
        ...
        paymentDate: salary.paymentDate ? moment(salary.paymentDate) : undefined,
        notes: salary.notes,
      });
      
      setCalculatedNetSalary(salary.netSalary);
    }
  }, [salary, open, form]);
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      message.error(error?.response?.data?.message || 'Lỗi cập nhật bảng lương');
    }
  };
  ...
  return (
    ...
    >
      <div className="mb-4 p-3 bg-gray-50 rounded">
        <Row gutter={16}>
          <Col span={12}>
            <div><strong>Nhân viên:</strong> {salary.employee.name}</div>
          </Col>
          <Col span={12}>
            <div><strong>Chức vụ:</strong> {salary.employee.position} - {salary.employee.department}</div>
          </Col>
        </Row>
      </div>
